...
clickText.addEventListener('click', () => {
    ...
    Object.keys(emotionData).forEach(emotion => {
        ...
        audio.play().catch(err => console.error(`오디오 재생 오류 (${emotion}):`, err));
    });
...
function showFinalResult() {
    ...
    finalColorBox.style.background = finalColor || 'white';

    mainEmotionText.textContent = mainEmotion || "감정이 감지되지 않았습니다.";

    Object.values(audioMap).forEach(audio => audio.volume = 0);
    if (audioMap[mainEmotion]) {
        audioMap[mainEmotion].volume = 1;
    }
